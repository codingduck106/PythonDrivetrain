...
from structs import Pose2dStruct
...
class SimDrivetrain(SimSwerveDrivetrain):
    ...
    def __init__(self, network_table: NetworkTable, drivetrain: SwerveDrivetrain, module_constants: list[SwerveModuleConstants]):
        ...
        self.sim_current_pose = network_table.getStructTopic("simulated pose", Pose2dStruct).publish()
    ...
    def periodic(self):
        ...
        self.sim_current_pose.set(Pose2dStruct(
            self.sim_pose.X(),
            self.sim_pose.Y(),
            self.sim_pose.rotation().radians()
        ))
